...
constexpr auto run_length_decode(TInIter first, TInIter last, TOutIter output)
 -> TOutIter
{
  while (first != last) {
...
constexpr auto run_length_decode(TRange&& range, TOutIter output)
 -> decltype(auto)
{
  return run_length_decode(std::cbegin(std::forward<TRange>(range)),
                           std::cend(std::forward<TRange>(range)),
                           output);
}

template<typename TInIter, typename TOutIter>
constexpr auto delta_decode(TInIter first, TInIter last, TOutIter output)
 -> TOutIter
{
  if (first != last) {
    auto prev_val = *first++;
...
{
  return delta_decode(std::cbegin(std::forward<TRange>(range)),
                      std::cend(std::forward<TRange>(range)),
                      output);
}

template<typename TInIter, typename TOutIter>
constexpr auto recursive_indexing_decode(TInIter first,
                                         TInIter last,
                                         TOutIter output) -> TOutIter
{
...
template<typename TRange, typename TOutIter>
constexpr auto recursive_indexing_decode(TRange&& range, TOutIter output)
 -> decltype(auto)
{
  return recursive_indexing_decode(std::cbegin(std::forward<TRange>(range)),
                                   std::cend(std::forward<TRange>(range)),
                                   output);
}
...
auto make_codec_header(TInIter iter, TInIter iter_end) noexcept -> CodecHeader<
 std::add_const_t<typename std::iterator_traits<TInIter>::value_type>>
{
  using DataByte =
   std::add_const_t<typename std::iterator_traits<TInIter>::value_type>;
  ...
  std::copy(std::next(iter, 8),
            data_begin,
            reinterpret_cast<std::uint8_t*>(parameter.data()));
  ...
  if constexpr (std::is_pointer_v<TInIter>) {
    ...
    header.encoded_data =
     gsl::span<DataByte>{data_begin.operator->(), iter_end.operator->()};
  }
...
constexpr auto
integer_decode(TInIter first, TInIter last, TOutIter output, TDivisor divisor)
 -> TOutIter
{
...
constexpr auto integer_decode(TRange&& range, TOutIter output, TDivisor divisor)
 -> decltype(auto)
{
  return integer_decode(std::cbegin(std::forward<TRange>(range)),
                        std::cend(std::forward<TRange>(range)),
                        output,
                        divisor);
}
...
{
  return make_codec_header(std::cbegin(std::forward<TRange>(range)),
                           std::cend(std::forward<TRange>(range)));
}
...
auto decode_header_type_1(CodecHeader<T> header) -> std::vector<float>
{
  if (header.strategy != 1) {
    const auto message =
     fmt::format("Wrong strategy type, expected 1 got {}.", header.strategy);
    throw std::invalid_argument{message};
  }

  if (header.encoded_data.size() % 4 != 0) {
    const auto message = fmt::format(
     "Data length {} is not a multiple of {}.", header.encoded_data.size(), 4);
    throw std::invalid_argument{message};
  }
  ...
  const auto floats_view = gsl::span<const std::uint32_t>{
   reinterpret_cast<const std::uint32_t*>(header.encoded_data.data()),
   data_size};

  output.reserve(data_size);
  std::transform(std::cbegin(floats_view),
                 std::cend(floats_view),
...
auto decode_header_type_2(CodecHeader<T> header) -> std::vector<std::int8_t>
{
  if (header.strategy != 2) {
    const auto message =
     fmt::format("Wrong strategy type, expected 2 got {}.", header.strategy);
    throw std::invalid_argument{message};
  }
  ...
  output.reserve(data_size);
  std::transform(std::cbegin(data_view),
                 std::cend(data_view),
                 std::back_inserter(output),
                 [](auto val) noexcept {
...
auto decode_header_type_3(CodecHeader<T> header) -> std::vector<std::int16_t>
{
  if (header.strategy != 3) {
    const auto message =
     fmt::format("Wrong strategy type, expected 3 got {}.", header.strategy);
    throw std::invalid_argument{message};
  }

  if (header.encoded_data.size() % 2 != 0) {
    const auto message = fmt::format(
     "Data length {} is not a multiple of {}.", header.encoded_data.size(), 2);
    throw std::invalid_argument{message};
  }
  ...
  const auto data_view = gsl::span<const std::int16_t>{
   reinterpret_cast<const std::int16_t*>(header.encoded_data.data()),
   data_size};
...
auto decode_header_type_4(CodecHeader<T> header) -> std::vector<std::int32_t>
{
  if (header.strategy != 4) {
    const auto message =
     fmt::format("Wrong strategy type, expected 4 got {}.", header.strategy);
    throw std::invalid_argument{message};
  }
  ...
  const auto data_size = header.encoded_data.size() / 4;
  const auto data_view = gsl::span<const std::int32_t>{
   reinterpret_cast<const std::int32_t*>(header.encoded_data.data()),
   data_size};
...
auto decode_header_type_5(CodecHeader<T> header) -> std::vector<std::string>
{
  if (header.strategy != 5) {
    const auto message =
     fmt::format("Wrong strategy type, expected 5 got {}.", header.strategy);
    throw std::invalid_argument{message};
  }

  const auto length_array = header.length;
  const auto chars_size = boost::endian::big_to_native(
   *reinterpret_cast<const std::int32_t*>(header.parameter.data()));
  const auto data_size = length_array * chars_size;

  if (header.encoded_data.size() != data_size) {
    const auto message =
     fmt::format("Data length is corrupt, {} expected, got {}.",
                 data_size,
                 header.encoded_data.size());
    throw std::invalid_argument{message};
  }
  ...
  output.reserve(data_size);
  for (auto it = std::cbegin(data_view); it != std::cend(data_view);
       std::advance(it, chars_size)) {
    const auto str =
     std::string{it.operator->(), static_cast<std::size_t>(chars_size)} + '\0';
...
auto decode_header_type_6(CodecHeader<T> header) -> std::vector<char>
{
  if (header.strategy != 6) {
    const auto message =
     fmt::format("Wrong strategy type, expected 6 got {}.", header.strategy);
    throw std::invalid_argument{message};
  }
...
auto decode_header_type_7(CodecHeader<T> header) -> std::vector<std::int32_t>
{
  if (header.strategy != 7) {
    const auto message =
     fmt::format("Wrong strategy type, expected 7 got {}.", header.strategy);
    throw std::invalid_argument{message};
  }
...
auto decode_header_type_8(CodecHeader<T> header) -> std::vector<std::int32_t>
{
  if (header.strategy != 8) {
    const auto message =
     fmt::format("Wrong strategy type, expected 8 got {}.", header.strategy);
    throw std::invalid_argument{message};
  }
...
auto decode_header_type_9(CodecHeader<T> header) -> std::vector<float>
{
  if (header.strategy != 9) {
    const auto message =
     fmt::format("Wrong strategy type, expected 9 got {}.", header.strategy);
    throw std::invalid_argument{message};
  }
  ...
  const auto divisor = boost::endian::big_to_native(
   *reinterpret_cast<const std::uint32_t*>(header.parameter.data()));
...
auto decode_header_type_10(CodecHeader<T> header) -> std::vector<float>
{
  ...
  if (header.strategy != strategy) {
    const auto message = fmt::format(
     "Wrong strategy type, expected {} got {}.", strategy, header.strategy);
    throw std::invalid_argument{message};
  }

  constexpr auto type_size = sizeof(std::uint16_t);
  if (header.encoded_data.size() % type_size != 0) {
    const auto message = fmt::format("Data length {} is not a multiple of {}.",
                                     header.encoded_data.size(),
                                     type_size);
    throw std::invalid_argument{message};
  }
  ...
  const auto data_size =
   header.encoded_data.size() /
   static_cast<decltype(header.encoded_data.size())>(type_size);
  const auto data_view = gsl::span<const std::int16_t>{
   reinterpret_cast<const std::int16_t*>(header.encoded_data.data()),
   data_size};
  ...
  recursive_indexing_decode(decoded_data,
                            std::back_inserter(decoded_recursive_indexing));
...
auto decode_header_type_12(CodecHeader<T> header) -> std::vector<float>
{
   ...
   *reinterpret_cast<const std::int32_t*>(header.parameter.data()));
  integer_decode(
   decoded_recursive_indexing, std::back_inserter(output), divisor);
...
auto decode_header_type_13(CodecHeader<T> header) -> std::vector<float>
{
  ...
  const auto data_size = header.encoded_data.size();
  const auto decoded_data = gsl::span<const std::int8_t>{
   reinterpret_cast<const std::int8_t*>(header.encoded_data.data()), data_size};
